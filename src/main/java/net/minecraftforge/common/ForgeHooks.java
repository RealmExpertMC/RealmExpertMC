...
public class ForgeHooks {
                ...
                (root, file) ->
                {
                    ...
                    if (!map.containsKey(key)) {
                        ...
                        try {
                            ...
                            RealmExpertMC.LOGGER.debug("Parsing error loading built-in advancement " + key, (Throwable) jsonparseexception);
                            return false;
                        } catch (IOException ioexception) {
                            RealmExpertMC.LOGGER.debug("Couldn't read advancement " + key + " from " + file, (Throwable) ioexception);
                            return false;
                        } finally {
    ...
    public static void sendRecipeBook(NetHandlerPlayServer connection, State state, List<IRecipe> recipes, List<IRecipe> display, boolean isGuiOpen, boolean isFilteringCraftable) {
        ...
        if (connection == null) {
            RealmExpertMC.LOGGER.warn("Server tried send recipe book to client but connection was null. Is client already disconnected?");
            return;
        }
