...
import static br.com.realmexpert.network.download.NetworkUtil.getConn;
import static br.com.realmexpert.network.download.NetworkUtil.getInput;
...
public class UpdateUtils {
    ...
    public static void versionCheck() {
        ...
        try {
            JsonElement root = new JsonParser().parse(new InputStreamReader(getInput("https://ci.codemc.io/job/RealmExpertMC/job/RealmExpertMC-1.12.2/lastSuccessfulBuild/api/json")));

            String jar_sha = RealmExpertMC.getVersion();
            ...
            if (jar_sha.equals(build_number))
                ...
                if (bRealmExpertMC("check_update_auto_download")) {
                    downloadFile("https://ci.codemc.io/job/RealmExpertMC/job/RealmExpertMC-1.12.2/lastSuccessfulBuild/artifact/projects/RealmExpertMC/build/libs/RealmExpertMC-" + build_number + "-server.jar", new File(getRealmExpertMCJar().getName()));
                    restartServer(new ArrayList<>(Arrays.asList("java", "-jar", getRealmExpertMCJar().getName())));
                }
            }
        } catch (Throwable e) {
    ...
    public static File getRealmExpertMCJar() {
        try {
            ...
            System.out.println("Can't found the RealmExpertMC jar !");
        }
        return null;
    }
