...
public class CraftPlayerProfile implements PlayerProfile {
    ...
    public boolean completeFromCache(boolean lookupName) {
        ...
        if (profile.getId() == null) {
            ...
            boolean isOnlineMode = server.isServerInOnlineMode() || RealmExpertMCConfig.isProxyOnlineMode();
            if (isOnlineMode) {
    ...
    public boolean complete(boolean textures) {
        ...
        if (isOnlineMode && (!isCompleteFromCache || textures && !hasTextures())) {
            try {
                ...
                if (RealmExpertMCConfig.instance.FailOnUnresolvedGameProfile.getValue()) {
                    e.printStackTrace();
                    throw e;
                }
            }
