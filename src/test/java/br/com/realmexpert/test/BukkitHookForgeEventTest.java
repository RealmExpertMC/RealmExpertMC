...
import br.com.realmexpert.api.event.BukkitHookForgeEvent;
import br.com.realmexpert.api.event.BukkitStateForgeEvent;
...
public class BukkitHookForgeEventTest implements Listener {
    ...
    public void test2(BukkitStateForgeEvent.PluginsEnable event) {
        if(event.getType().equals(PluginLoadOrder.POSTWORLD) && event.getServer().getPluginManager().getPlugin("RealmExpertMC") != null){
            event.getServer().getConsoleSender().sendMessage("Thank you for using RealmExpertMC!");
        }
    }
}
